...
def ball_animation():
    ...
    if ball.top <=0 or ball.bottom >= screen_height:
        ball_speed_y *= -1 #if top of the ball is0 or bottom the ball is equal to screen height then reverse vertical ball speed

    if ball.left <=0 or ball.right >= screen_width:
        ball_restart()

    if ball.colliderect(player) or ball.colliderect(opponent):
        ball_speed_x *= -1 #if left of the ball is 0 or right of the ball is equal to screen width then revrse horizontal ball speed
