...
castleSquares = {g.Piece.white: [[6, 7], [0, 1, 2]], g.Piece.black: [[62, 63], [56, 57, 58]]}
...
requiredSquares = {g.Piece.white: [[5, 6], [2, 3]], g.Piece.black: [[61, 62], [58, 59]]}
...
def canCastle(endPos, colour):
    ...
    for i in requiredSquares[colour][castleSide]:
        if i in g.attacked:
            return False
...
def isCastling(endPos, colour) -> bool:
    for i in castleSquares[colour]:
        if endPos in i:
            return (True and g.heldPiece[0] & g.pieceMask == g.Piece.King)
